class Parser {
  ...
  factor() {
    ...
    // Manejar números negativos: Si el token actual es un signo menos, lo trata como negativo
    if (token.type === 'MINUS') {
      this.eat('MINUS');  // Avanzar si se encuentra el signo menos
      const node = this.factor();  // Evaluar el siguiente número como factor
      return { type: 'NEGATE', value: node };  // Crear un nodo de negación
    }
